"""
DECISIO PDF Generator V10 — DocRaptor + HTML/CSS
Objectif : 9/10 niveau McKinsey premium
"""
import re
import requests
...
def _score_bar(score_str):
    """Transforme '8.5' en barre de progression visuelle HTML"""
    try:
  ...
  <span class="score-bar-num" style="color:{color}">{score_str}/10</span>
</div>'''
    except:
        return f'<span>{score_str}/10</span>'
...
def _partie_break(num, titre, color):
    """Page séparatrice entre les 3 parties — fond navy, numéro grand"""
    return f'''<div class="partie-break" style="background:{color}">
  <div class="partie-num">0{num}</div>
  <div class="partie-titre">{titre}</div>
  <div class="partie-rule"></div>
  <div class="partie-sub">MÉTHODE D3™ · DECISIO AGENCY</div>
</div>'''
...
def parse_report(text):
    ...
    while i < len(lines):
        ...
        # H1 — détecte les parties D3 pour insérer page séparatrice
        m = re.match(r'^#{1}\s+(.+)', line)
        if m:
            txt = clean(m.group(1))
            t = txt.upper()
            if 'PARTIE 1' in t or 'DIAGNOSTIC' in t:
                add(_partie_break(1, 'DIAGNOSTIC', RED_ACC))
            elif 'PARTIE 2' in t or 'DÉCISION' in t or 'DECISION' in t:
                add(_partie_break(2, 'DÉCISION', ORANGE))
            elif 'PARTIE 3' in t or 'DÉPLOIEMENT' in t or 'DEPLOIEMENT' in t:
                add(_partie_break(3, 'DÉPLOIEMENT', GREEN_ACC))
            else:
                add(f'<h1 class="h1">{txt}</h1>')
            i += 1; continue

        # H2 — section header coloré
        ...
        # Options A/B/C avec score visuel
  ...
  <div class="option-body">
    ...
    <div class="option-detail">{detail.strip()}</div>
    {score_html}
  </div>
</div>''')
            i += 1; continue
        ...
        if line.startswith('|') and i+1 < len(lines):
            ...
            add(f'<table class="report-table"><tbody>{html_rows}</tbody></table>')
            continue
...
body {{
  ...
  line-height: 1.75; background: white;
}}
...
.cover-left {{
  width: 40%; background: {NAVY};
  padding: 14mm 9mm 10mm 10mm;
...
.cover-method {{
  font-size: 6.5pt; font-weight: 700; color: {GOLD};
  letter-spacing: 0.18em; text-transform: uppercase; margin-top: 3mm;
}}
.cover-tagline {{
  font-size: 6.5pt; color: rgba(255,255,255,0.4);
  letter-spacing: 0.07em; margin-top: 2mm;
}}
...
.pillar-num {{
  ...
  color: {NAVY}; flex-shrink: 0;
}}
.pillar-title {{ font-size: 8.5pt; font-weight: 700; color: white; }}
...
.cover-badge {{
  align-self: flex-end; background: {NAVY};
  color: {GOLD}; font-size: 6.5pt; font-weight: 700;
  letter-spacing: 0.1em; padding: 2.5mm 5mm; border-radius: 1.5mm;
}}
.cover-client-name {{
  font-family: 'Playfair Display', Georgia, serif;
  font-size: 26pt; font-weight: 700;
  color: {NAVY}; line-height: 1.1; margin-top: 16mm;
}}
...
.cover-info-box {{
  ...
  border-left: 3px solid {GOLD}; border-radius: 3mm;
...
.cover-price {{
  font-family: 'Playfair Display', Georgia, serif;
  font-size: 24pt; font-weight: 700; color: {NAVY}; line-height: 1;
}}
...
.report-title {{
  font-family: 'Playfair Display', Georgia, serif;
  font-size: 18pt; font-weight: 700; color: {NAVY};
  text-align: center; line-height: 1.25; margin-bottom: 3mm;
}}
.report-subtitle {{
...
.section-header {{
  background: {LIGHT_BG}; border-left: 6px solid {NAVY};
  padding: 4.5mm 5mm; margin: 11mm 0 5mm;
  page-break-after: avoid;
}}
.section-label {{ font-size: 10.5pt; font-weight: 700; letter-spacing: 0.04em; }}
.h3 {{
  font-size: 10.5pt; font-weight: 700; color: {NAVY};
  margin: 7mm 0 2.5mm; padding-bottom: 1.5mm;
  border-bottom: 2px solid {GOLD};
  page-break-after: avoid;
  display: inline-block; padding-right: 8mm;
}}
.h4 {{
  font-size: 10pt; font-weight: 600; color: {NAVY};
  margin: 5mm 0 1.5mm; page-break-after: avoid;
}}
.body {{
  font-size: 10.5pt; color: {TEXT}; line-height: 1.8;
  text-align: justify; margin-bottom: 4.5mm;
  orphans: 3; widows: 3;
}}
...
.report-list li {{
  ...
  color: {TEXT}; margin-bottom: 3mm;
}}
ul.report-list {{ list-style: none; padding-left: 0; }}
ul.report-list li::before {{
  content: '●'; color: {GOLD};
  font-size: 7pt; margin-right: 3mm; vertical-align: middle;
}}

/* ── KV ───────────────────────────────────────────── */
.kv-row {{
  display: flex; align-items: baseline; gap: 3mm;
  padding: 3mm 4mm; background: {LIGHT_BG};
  border-bottom: 1px solid {BORDER};
  border-left: 3px solid {GOLD};
}}
.kv-label {{
  font-size: 9pt; font-weight: 700; color: {NAVY};
  min-width: 52mm; flex-shrink: 0;
}}
.kv-value {{ font-size: 9.5pt; color: {TEXT}; }}

/* ── VÉRITÉ FONDAMENTALE ──────────────────────────── */
.verite-block {{
  background: {NAVY}; border-left: 5px solid {GOLD};
  padding: 5mm 7mm; margin: 7mm 0;
  page-break-inside: avoid;
}}
.verite-label {{
  font-size: 7pt; font-weight: 700; color: {GOLD};
  letter-spacing: 0.15em; text-transform: uppercase; margin-bottom: 3mm;
}}
.verite-text {{
...
/* ── BLOCKQUOTE ───────────────────────────────────── */
.bq {{
  background: {LIGHT_BG}; border-left: 3px solid {BLUE_ACC};
  padding: 3.5mm 5mm; margin: 4mm 0;
  font-size: 10pt; font-style: italic; color: #333;
  page-break-inside: avoid;
}}

/* ── OPTIONS A/B/C avec barre score ──────────────── */
.option-card {{
  display: flex; align-items: flex-start; gap: 4mm;
  background: {LIGHT_BG}; border: 1px solid {BORDER};
  border-left: 6px solid; border-radius: 2mm;
...
.option-detail {{ font-size: 8.5pt; color: {MUTED}; margin-bottom: 2mm; }}
...
.score-bar-track {{
  flex: 1; height: 5px; background: {BORDER};
...
.score-bar-num {{
  font-size: 9pt; font-weight: 700; min-width: 30px;
}}
...
.message-label {{
  font-size: 7.5pt; font-weight: 700; color: {BLUE_ACC};
...
.report-table th {{
  ...
  padding: 3.5mm 4mm; text-align: left;
  border-top: 2.5px solid {GOLD};
}}
.report-table td {{
  padding: 3mm 4mm; border-bottom: 1px solid {BORDER};
...
/* ── GRAPHIQUE ────────────────────────────────────── */
.chart-container {{ margin: 5mm 0; page-break-inside: avoid; }}
.chart-inner {{
  display: flex; align-items: flex-end; gap: 6mm;
  height: 52mm; padding: 0 4mm;
...
.chart-bars {{
  ...
  gap: 2mm; height: 44mm; width: 100%; justify-content: center;
}}
.bar-wrap {{
  display: flex; flex-direction: column;
  align-items: center; justify-content: flex-end;
  height: 100%; min-width: 13mm;
}}
...
.bar-val {{
  ...
  color: {NAVY}; text-align: center; margin-bottom: 1mm;
}}
.bar-val.projected {{ color: #7A6020; }}
.chart-label {{
  font-size: 8pt; font-weight: 600; color: {MUTED};
  text-align: center; margin-top: 2.5mm;
  padding-top: 2mm; border-top: 1px solid {BORDER}; width: 100%;
}}
.chart-legend {{
  display: flex; gap: 6mm; justify-content: center;
  margin-top: 3mm; font-size: 8pt; color: {MUTED};
}}
